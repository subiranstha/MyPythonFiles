import numpy as np
import matplotlib.pyplot as plt
import pandas as pd
from sklearn.preprocessing import StandardScaler
from sklearn.svm import SVR
from joblib import dump
from sklearn.model_selection import train_test_split
from sklearn.metrics import mean_squared_error, mean_absolute_error, r2_score, explained_variance_score

import matplotlib.pyplot as plt
import seaborn as sns
fileName ='C:\\Users\\Subiran\\Desktop\\My Dataset\\AllImages.xlsx'

xl_file = pd.ExcelFile(fileName)

df_samples = pd.read_excel(fileName, engine='openpyxl')

X = df_samples.iloc[:, 1 : -1]
Y = df_samples.iloc[:, -1]
X = np.array(X)
Y = np.array(Y)
Y = Y.reshape(-1, 1)
StdS_X = StandardScaler()
X = StdS_X.fit_transform(X)
StdS_y = StandardScaler()
Y = StdS_y.fit_transform(Y)
X_train, X_test, Y_train, Y_test = train_test_split(X, Y, test_size=0.30, random_state= 30)

# We directly go to Support vector regressor:

regressor = SVR(kernel='rbf').fit(X_train, Y_train)

y_hat = regressor.predict(X_test) # one d array
y_hat = np.array(y_hat)
predictions = pd.DataFrame({'y_test': Y_test.flatten(), 'y_hat': y_hat.flatten()})
print(predictions.head())
print(regressor.score(X_test, Y_test))




#sns.scatterplot(x = Y_test.flatten(), y = y_hat.flatten(), alpha = 0.6)
#sns.lineplot(x=Y_test.flatten(), y=Y_test.flatten())
#sns.lineplot(x=y_hat.flatten(), y=y_hat.flatten())

plt.scatter(Y_test, y_hat)
plt.xlabel("Actual or true value", fontsize = 14)
plt.ylabel("Predicted value", fontsize = 14)
plt.title("Actual vs predicted", fontsize = 14)
plt.show()

